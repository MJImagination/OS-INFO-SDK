package util.SevenZip.Compression.LZMA;

//import util.SevenZip.Compression.LZ.BinTree;
//import util.SevenZip.Compression.RangeCoder.BitTreeEncoder;
//import util.SevenZip.ICodeProgress;
...
public class Encoder
{
	...
	class LiteralEncoder
	{
		class Encoder2
		{
			...
			public void Init() { util.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(m_Encoders); }



			public void Encode(util.SevenZip.Compression.RangeCoder.Encoder rangeEncoder, byte symbol) throws IOException
			{
				int context = 1;
				for (int i = 7; i >= 0; i--)
				{
			...
			public void EncodeMatched(util.SevenZip.Compression.RangeCoder.Encoder rangeEncoder, byte matchByte, byte symbol) throws IOException
			{
				int context = 1;
				boolean same = true;
				for (int i = 7; i >= 0; i--)
			...
			public int GetPrice(boolean matchMode, byte matchByte, byte symbol)
			{
				...
				if (matchMode)
				{
					for (; i >= 0; i--)
					{
						int matchBit = (matchByte >> i) & 1;
						int bit = (symbol >> i) & 1;
						price += util.SevenZip.Compression.RangeCoder.Encoder.GetPrice(m_Encoders[((1 + matchBit) << 8) + context], bit);
						context = (context << 1) | bit;
						if (matchBit != bit)
						{
							i--;
				...
				{
					int bit = (symbol >> i) & 1;
					price += util.SevenZip.Compression.RangeCoder.Encoder.GetPrice(m_Encoders[context], bit);
					context = (context << 1) | bit;
				}
				return price;
			}
	...
	class LenEncoder
	{
		...
		{
			util.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_choice);
		...
		public void Encode(util.SevenZip.Compression.RangeCoder.Encoder rangeEncoder, int symbol, int posState) throws IOException
		{
		...
		{
			int a0 = util.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_choice[0]);
			int a1 = util.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_choice[0]);
			int b0 = a1 + util.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_choice[1]);
			int b1 = a1 + util.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_choice[1]);
			int i = 0;
	...
	util.SevenZip.Compression.RangeCoder.Encoder _rangeEncoder = new util.SevenZip.Compression.RangeCoder.Encoder();
	...
	void Init()
	{
		...
		util.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_isMatch);
		util.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_isRep0Long);
		util.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_isRep);
		util.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_isRepG0);
		util.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_isRepG1);
		util.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_isRepG2);
		util.SevenZip.Compression.RangeCoder.Encoder.InitBitModels(_posEncoders);
	...
	{
		return util.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isRepG0[state]) +
				util.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
	}
	...
	int GetPureRepPrice(int repIndex, int state, int posState)
	{
		...
		if (repIndex == 0)
		{
			price = util.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isRepG0[state]);
			price += util.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isRep0Long[(state << Base.kNumPosStatesBitsMax) + posState]);
		}
		else
		{
			price = util.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isRepG0[state]);
			if (repIndex == 1)
				price += util.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isRepG1[state]);
			else
			{
				price += util.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isRepG1[state]);
				price += util.SevenZip.Compression.RangeCoder.Encoder.GetPrice(_isRepG2[state], repIndex - 2);
			}
		}
		return price;
	}
	...
	int GetOptimum(int position) throws IOException
	{
		...
		_optimum[1].Price = util.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isMatch[(_state << Base.kNumPosStatesBitsMax) + posState]) +
				_literalEncoder.GetSubCoder(position, _previousByte).GetPrice(!Base.StateIsCharState(_state), matchByte, currentByte);
		_optimum[1].MakeAsChar();

		int matchPrice = util.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isMatch[(_state << Base.kNumPosStatesBitsMax) + posState]);
		int repMatchPrice = matchPrice + util.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isRep[_state]);
		...
		int normalMatchPrice = matchPrice + util.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isRep[_state]);
		...
		while (true)
		{
			...
			int curAnd1Price = curPrice +
				util.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isMatch[(state << Base.kNumPosStatesBitsMax) + posState]) +
			...
			matchPrice = curPrice + util.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isMatch[(state << Base.kNumPosStatesBitsMax) + posState]);
			repMatchPrice = matchPrice + util.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isRep[state]);
			...
			if (!nextIsChar && matchByte != currentByte)
			{
				...
				if (lenTest2 >= 2)
				{
					...
					int nextRepMatchPrice = curAnd1Price +
						util.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) +
						util.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isRep[state2]);
					{
				...
				if (lenTest < numAvailableBytesFull)
				{
					...
					if (lenTest2 >= 2)
					{
						...
						int curAndLenCharPrice =
								...
								util.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]) +
						...
						int nextMatchPrice = curAndLenCharPrice + util.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isMatch[(state2 << Base.kNumPosStatesBitsMax) + posStateNext]);
						int nextRepMatchPrice = nextMatchPrice + util.SevenZip.Compression.RangeCoder.Encoder.GetPrice1(_isRep[state2]);
			...
			if (newLen >= startLen)
			{
				normalMatchPrice = matchPrice + util.SevenZip.Compression.RangeCoder.Encoder.GetPrice0(_isRep[state]);
	...
	void FillDistancesPrices()
	{
		...
		for (int lenToPosState = 0; lenToPosState < Base.kNumLenToPosStates; lenToPosState++)
		{
			...
			for (posSlot = Base.kEndPosModelIndex; posSlot < _distTableSize; posSlot++)
				_posSlotPrices[st + posSlot] += ((((posSlot >> 1) - 1) - Base.kNumAlignBits) << util.SevenZip.Compression.RangeCoder.Encoder.kNumBitPriceShiftBits);
